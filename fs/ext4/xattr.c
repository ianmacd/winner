...
extern void dlog_hook_security(struct inode *);
#endif
...
#ifdef CONFIG_PROC_DLOG
		if (error == 0 && strcmp(name, XATTR_SELINUX_SUFFIX) == 0) {
			if (strcmp(EXT4_SB(inode->i_sb)->s_es->s_volume_name,
						"data") == 0)
				dlog_hook_security(inode);
		}
#endif
	} else {
		error = 0;
