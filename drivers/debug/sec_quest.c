...
static struct param_quest_t param_quest_data;
...
static void print_param_quest_data()
{
...
void set_ddr_scan_result_for_smd()
{
	int ddr_result = DDRTEST_INCOMPLETED;
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	if( param_quest_data.quest_step == CAL_QUEST ) {
		if ( (fd = sys_open(UEFI_QUESTRESULT_FILE, O_RDONLY, 0))>=0 ) {
			...
			// if uefi logs which was not parsed exist, backup previous logs first and move current uefi logs to cal directory
			QUEST_PRINT("%s : cal_step and uefi logs exists, so backup and move uefi logs\n", __func__);

			// if hlos has been already failed, do not backup cal logs because repeating hlos&uefi will be ended soon
			...
			// the reasons are :
			// normal status
			//   1. quest_uefi was not triggered
			//   2. questresult has already been moved to /data/log/quest/CAL
			// abnormal status
			//   1. unknown exception happened during quest_uefi
			//   2. mount fail or sys_open fail
			//   3. unknown
			//QUEST_PRINT("%s : questresult logs does not exist (fd=%d)\n", __func__, fd);
		//}
	}else 
		QUEST_PRINT("%s : now step is not cal quest", __func__);
	set_fs(old_fs);
#endif	
