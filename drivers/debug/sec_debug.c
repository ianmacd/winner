...
EXPORT_SYMBOL(get_pa_dump_sink);
...
void sec_debug_update_restart_reason(const char *cmd, const int in_panic, const int restart_mode)
{
	struct __magic {
		const char *cmd;
...
struct __upload_cause upload_cause_st[] = {
	...
	{ "taking too long", UPLOAD_CAUSE_KERNEL_PANIC, SEC_STRNSTR },
	...
	{ "cachecoherency_test", UPLOAD_CAUSE_QUEST_CACHECOHERENCY, SEC_STRNCMP },
	...
	{ "qmesa_ddr_test_cache_centric", UPLOAD_CAUSE_QUEST_QMESACACHE, SEC_STRNCMP },
	...
	{ "nad_fail", UPLOAD_CAUSE_QUEST_FAIL, SEC_STRNCMP },
#endif
};

void sec_debug_upload_cause_str(enum sec_debug_upload_cause_t type, char *str)
{
	size_t i = 0;

	if (type == UPLOAD_CAUSE_KERNEL_PANIC) {
		strcpy(str, "kernel_panic");
	} else if (type == UPLOAD_CAUSE_INIT) { 
		strcpy(str, "unknown_error");
	} else if (type == UPLOAD_CAUSE_NON_SECURE_WDOG_BARK) {
		strcpy(str, "watchdog_bark");
	} else {
		for (i = 0; i < ARRAY_SIZE(upload_cause_st); i++) {
			if (type == upload_cause_st[i].type) {
				strncpy(str, upload_cause_st[i].msg, strlen(upload_cause_st[i].msg));
				return;
			}
		}
		strcpy(str, "unknown_reset");
	}
}
...
#endif
		if (!force_upload) {
			// can not enter into ramdump mode
			// temporary block till find root cause(case 03613371)
