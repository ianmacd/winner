...
void muic_detect_dev(muic_data_t *pmuic)
{
	...
	if (pmuic->afc_water_disable) {
		if (lpcharge) {
			pr_info("%s: Water is detected in lpcharging, Skip switching to audio path. \n", __func__);
		} else {
			pr_info("%s: The attach Intr occured but water is detected, set GPIO to high and switch to audio path. \n", __func__);
			muic_GPIO_control(1);
			switch_to_audio(pmuic);
			goto check_vb;
		}	
	}
#endif
	...
	if (pmuic->opmode & OPMODE_CCIC) {
		if (!mdev_continue_for_TA_USB(pmuic, new_dev)) {
                        if ( (pmuic->vps.s. val1 == 0) && (pmuic->vps.s. val2 == 0) && (pmuic->vps.s. val3 == 0) ) {
                                pmuic->is_afc_device = 0;
                                printk("%s: pmuic->is_afc_device: %d\n", __func__, pmuic->is_afc_device);
                        }
			goto check_vb;
		}
	}
