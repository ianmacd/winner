...
int sysboot_i2c_erase(struct cam_ois_ctrl_t *o_ctrl, uint32_t address, size_t len)
{
	...
	for (retry = 0; retry < BOOT_I2C_SYNC_RETRY_COUNT; ++retry)
	{
		...
		if (erase.page != 0xFFFF)
		{
			...
			CAM_INFO(CAM_OIS, "&pbuf[ix] %p,xmit %p", &pbuf[ix], xmit);
...
#if defined(CONFIG_SEC_WINNERLTE_PROJECT) || defined(CONFIG_SEC_WINNERX_PROJECT) || defined(CONFIG_SEC_DAVINCIXQ_PROJECT)
