 ...
 */
void make_self_dispaly_img_cmds_FA7(struct samsung_display_driver_data *vdd,
...
static void self_move_on(struct samsung_display_driver_data *vdd, int enable)
{
	if (IS_ERR_OR_NULL(vdd)) {
		LCD_ERR("vdd is null or error\n");
		return;
	...
	if (enable) {
		...
		/* S6E3FA7 does not support self move index reset */
		ss_send_cmd(vdd, TX_SELF_MOVE_2C_SYNC_OFF);
	}
...
static void self_icon_img_write(struct samsung_display_driver_data *vdd)
{
	...
	ss_set_exclusive_tx_packet(vdd, TX_SELF_ICON_SIDE_MEM_SET, 1);
	...
	ss_set_exclusive_tx_packet(vdd, TX_SELF_ICON_SIDE_MEM_SET, 0);
...
static int self_aclock_set(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO("Self Analog Clock Enable(%d), x(%d), y(%d), rot(%d)\n",
		vdd->self_disp.sa_info.en, vdd->self_disp.sa_info.pos_x, 
		vdd->self_disp.sa_info.pos_y, vdd->self_disp.sa_info.rotate);

	if (!vdd->self_disp.sa_info.en)
		goto skip_update;
	...
	/* Rotation Changed Case */
	if (prev_rot != sa_info.rotate) {
		LCD_INFO("Rotation Changed\n");
		cmd_pload[2] &= ~(BIT(1)); /* Disable SC_DISP_ON */
		ss_send_cmd(vdd, TX_SELF_ACLOCK_ON);
		usleep_range(1000*1000, 1000*1000); /* 1sec delay */
		cmd_pload[2] |= BIT(1); /* Enable SC_DISP_ON */
		prev_rot = sa_info.rotate;
	}

	vdd->self_disp.time_set = true;

skip_update:
	self_aclock_on(vdd, vdd->self_disp.sa_info.en);
...
static void self_dclock_img_write(struct samsung_display_driver_data *vdd)
{
	...
	ss_set_exclusive_tx_packet(vdd, TX_LEVEL1_KEY_ENABLE, 0);
	ss_set_exclusive_tx_packet(vdd, TX_SELF_DCLOCK_SIDE_MEM_SET, 0);
...
static long self_display_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_SELF_MOVE_EN:
		self_move_on(vdd, true);
		break;
	case IOCTL_SELF_MOVE_OFF:
	case IOCTL_SELF_MOVE_RESET:
		/* FA7 does not support move reset, use move off only */
		self_move_on(vdd, false);
		break;
	case IOCTL_SET_ICON:
			 ...
			 size_t count, loff_t *ppos)
{
	...
	case FLAG_SELF_MOVE:
		...
		break;
	case FLAG_SELF_MASK:
		make_self_dispaly_img_cmds_FA7(vdd, TX_SELF_MASK_IMAGE, op);
		vdd->self_disp.operation[FLAG_SELF_MASK].select = true;
		break;
	case FLAG_SELF_ICON:
		make_self_dispaly_img_cmds_FA7(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[FLAG_SELF_ICON].select = true;
		break;
	case FLAG_SELF_GRID:
		// GRID has no image data..
		break;
	case FLAG_SELF_ACLK:
		make_self_dispaly_img_cmds_FA7(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[FLAG_SELF_ACLK].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_self_dispaly_img_cmds_FA7(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = true;
...
int self_display_init_FA7(struct samsung_display_driver_data *vdd)
{
	int ret = 0;
