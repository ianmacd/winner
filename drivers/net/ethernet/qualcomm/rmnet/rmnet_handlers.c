...
static long gro_flush_time __read_mostly = 12500L;
				     ...
				     struct rmnet_port *port)
{
	...
	if(rmnet_data_tx_aggr_enabled)
		return;
	
	if (unlikely(port->flush_time.tv_sec == 0)) {
		getnstimeofday(&port->flush_time);
		return;
	}

	getnstimeofday(&(curr_time));
	diff = timespec_sub(curr_time, port->flush_time);
...
rmnet_deliver_skb(struct sk_buff *skb, struct rmnet_port *port)
{
	...
	if (port->data_format & RMNET_INGRESS_FORMAT_DL_MARKER) {
		if (!rmnet_check_skb_can_gro(skb) &&
			...
			rmnet_optional_gro_flush(napi, port);
		} else {
