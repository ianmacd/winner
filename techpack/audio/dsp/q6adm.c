...
static bool port_supports_custom_mch_map(int port_id)
{
	switch (port_id) {
			  ...
			  int channel_mode, int port_id)
{
	...
	if ((open->dev_num_channel > 2) && multi_ch_maps[idx].set_channel_map &&
		port_supports_custom_mch_map(port_id)) {
...
void adm_copp_mfc_cfg(int port_id, int copp_idx, int dst_sample_rate)
{
	...
	rc = adm_arrange_mch_map(&open, ADM_PATH_PLAYBACK,
		mfc_cfg.num_channels, q6audio_get_port_id(port_id));
	if (rc < 0) {
